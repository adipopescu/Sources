...
#endif
{
  ...
  Lp.i_r = -1;
#if 1
printf("\nTrying to add Spoly from:\n");pWrite(p);pWrite(pSig);pWrite(strat->S[i]);pWrite(sSigMult);
#endif
#ifdef KDEBUG
  Lp.ecart=0; Lp.length=0;
#endif
  ...
  {
    number lc1,lc2;
    lc1 = nCopy(p_GetCoeff(strat->S[i], currRing));
    lc2 = nCopy(p_GetCoeff(p, currRing));
    ksCheckCoeff(&lc1, &lc2, currRing->cf);
    p_SetCoeff(m1, lc1, currRing);
    p_SetCoeff(m2, lc2, currRing);
...
printf("\nIn Spoly: m1, m2:\n");pWrite(m1);pWrite(m2);
  ...
  pSigMultNegSev = ~p_GetShortExpVector(pHead(pSigMult),currRing);
  sSigMult = currRing->p_Procs->pp_Mult_mm(sSigMult,m2,currRing);
  sSigMultNegSev = ~p_GetShortExpVector(pHead(sSigMult),currRing);
#if 0
//#ifdef HAVE_RINGS
  if(rField_is_Ring(currRing))
  {
    if(!nGreaterZero(p_GetCoeff(pSigMult, currRing)))
    {
        pNeg(pSigMult);
    }
    if(!nGreaterZero(p_GetCoeff(sSigMult, currRing)))
    {
        pNeg(sSigMult);
    }
  }
...
//#if 1
  ...
  pWrite(sSigMult);
    ...
    #if ADIDEBUG
    printf("\nPair from\n");
    pWrite(p);pWrite(pSig);pWrite(strat->S[i]);pWrite(strat->sig[i]);
    printf("\nThe two signatures in the spoly computations are:\n");
    pWrite(pSigMult);
    pWrite(sSigMult);
    #endif
  if(sigCmp==0)
  {
      //printf("!!!!   EQUAL SIGS - Delete   !!!!\n");pWrite(Lp.p);
  ...
  */
  ...
  else
  {
    ...
    {
      assume(!rIsPluralRing(currRing));
      Lp.p = ksCreateShortSpolySig(strat->S[i], p, minus, strat->tailRing);
      printf("\nVia ksCreateShortSpoly, minus = %i\n",minus);
#if MYTEST
      if (TEST_OPT_DEBUG)
      {
        PrintS("enterOnePairNormal::\n strat->S[i]: "); pWrite(strat->S[i]);
  ...
  {
    if (Lp.lcm!=NULL) pLmFree(Lp.lcm);
    int pos = posInSyz(strat, pHead(Lp.sig));
  ...
  else
  {
    ...
    #ifdef HAVE_RINGS
    if (strat->rewCrit3(pHead(Lp.sig),~Lp.sevSig,Lp.p,strat,strat->sl+1)) {
      printf("\n                            rewCrit3 DELETED!!!!!\n");
      pLmFree(Lp.lcm);
      pDelete(&Lp.sig);
      Lp.lcm=NULL;
...
void enterOneStrongPolySig (int i, poly p, poly pSig, int, int ecart, int isFromQ, kStrategy strat, int atR = -1)
{
  ...
  unsigned long pSigMultNegSev,sSigMultNegSev;
  Lp.i_r = -1;
  printf("\nenteronestrongpairSig tries to add GCD poly from\n");pWrite(p);pWrite(strat->S[i]);
  ...
  Lp.lcm = pInit();
  pSetCoeff0(Lp.lcm, n_Lcm(pGetCoeff(p), pGetCoeff(strat->S[i]), currRing->cf));
  k_GetStrongLeadTerms(p,strat->S[i],currRing,m1,m2,gcd,currRing);
#ifndef HAVE_RATGRING
  pLcm(p,strat->S[i],Lp.lcm);
#elif defined(HAVE_RATGRING)
  //  if (rIsRatGRing(currRing))
  ...
  // set coeffs of multipliers m1 and m2
  number lc1,lc2, lcgcd;
  #ifdef HAVE_RINGS
  if(rField_is_Ring(currRing))
  {
    lcgcd = n_ExtGcd(pGetCoeff(p), pGetCoeff(strat->S[i]), &lc1, &lc2, currRing->cf);
printf("\nCheck This:\n");
    n_Print(lcgcd,currRing->cf);printf(" ");n_Print(lc1,currRing->cf);printf(" ");n_Print(lc2,currRing->cf);pWrite(p);pWrite(strat->S[i]);
    //lc1 = nCopy(p_GetCoeff(strat->S[i], currRing));
    //lc2 = nCopy(p_GetCoeff(p, currRing));
    //ksCheckCoeff(&lc1, &lc2, currRing->cf);
    //printf("\n              enteronestrongpair:\n");n_Print(pGetCoeff(p), currRing->cf);n_Print(pGetCoeff(strat->S[i]), currRing->cf);n_Print(lc1, currRing->cf);n_Print(lc2, currRing->cf);
    if (nIsZero(lc1) || nIsZero(lc2))  // evtl. durch divBy tests ersetzen
    {
...
//#if 1
  ...
  */
  ...
  // store from which element this pair comes from for further tests
  //Lp.from = strat->sl+1;
  printf("\npSigMult und sSigMult\n");
  pWrite(pSigMult);pWrite(sSigMult);
  #ifndef HAVE_RINGS
  if(sigCmp==currRing->OrdSgn)
  {
    // pSig > sSig
  ...
  #endif
  #ifdef HAVE_RINGS
  printf("\nIn Enteronestrongpoly: We have these two elements and this is the sig:\n");
  pWrite(Lp.p);pWrite(Lp.sig);pWrite(p);pWrite(pSig);pWrite(strat->S[i]);pWrite(strat->sig[i]);
  #endif
  if (Lp.p == NULL)
  {
    if (Lp.lcm!=NULL) pLmFree(Lp.lcm);
  ...
  {
    printf("\nRight before rewCrit3 in enteronespoly\n");
    // testing by rewCrit3 = Arris Rewritten Criterion (for F5 nothing happens!)
    #ifdef HAVE_RINGS
    if (strat->rewCrit3(pHead(Lp.sig),~Lp.sevSig,Lp.p,strat,strat->sl+1)) {
      printf("\nDeleted! in rewCrit3\n");
...
void enterFirstSpoly (int f,int k,int ecart,int isFromQ,kStrategy strat, int atR = -1)
{
  ...
  int many = 0;
    ...
    else
    {
      for (j=f+1; j<=k; j++)
      {
        ...
        {
          new_pair=TRUE;
          strat->enterOnePair(j,h,ecart,isFromQ,strat, atR);many++;
        //Print("j:%d, Ll:%d\n",j,strat->Ll);
        }
      }
    }
    printf("\nFor the following element %i pairs were added:\n",many);pWrite(h);
...
void enterExtendedSpolySig(poly h, poly hSig, int hFrom, kStrategy strat)
{
  if (nIsOne(pGetCoeff(h))) return;
  number gcd;
  bool go = false;
...
void superenterpairsSig (poly h,poly hSig,int hFrom,int k,int ecart,int pos,kStrategy strat, int atR)
{
  assume (rField_is_Ring(currRing));
  // enter also zero divisor * poly, if this is non zero and of smaller degree
  //    !!!!!!! ACTIVATE IT WHEN TESTING OVER Z_n
  //if (!(rField_is_Domain(currRing))) 
    //    enterExtendedSpolySig(h, hSig, hFrom, strat);
  int i;
  i = strat->Ll;
    ...
    if (rField_is_Ring(currRing))
    {
      ...
      loop
      {
        if (an >= en-1)
        {
          ...
          if (cmp == 1)  return an;
          if (cmp == -1) return en;
          if (n_DivBy(pGetCoeff(p.p), pGetCoeff(set[an].p), currRing->cf)) return en;
          return an;
        }
        i = (an+en) / 2;
        ...
        {
          if (n_DivBy(pGetCoeff(p.p), pGetCoeff(set[i].p), currRing->cf)) an = i;
          else en = i;
        }
      }
    }
...
if(rField_is_Ring(currRing))
{
  ...
  if (pLmCmp(set[length].sig, p->sig) == 1) //1, since this will only be used in global  orderings
        return length+1;
  number np, nset;
  np = n_Copy(pGetCoeff(p->sig), currRing->cf);
  nset = n_Copy(pGetCoeff(set[length].sig), currRing->cf);
  if(!n_GreaterZero(np, currRing->cf))  n_InpNeg(np, currRing->cf);
  if(!n_GreaterZero(nset, currRing->cf))  n_InpNeg(nset, currRing->cf);
  ...
  loop
  {
    if (an >= en-1)
    {
      ...
      if (pLmCmp(set[an].sig,p->sig) == 1) 
        return en;
      nset = n_Copy(pGetCoeff(set[an].sig), currRing->cf);
      if(!n_GreaterZero(nset, currRing->cf))  n_InpNeg(nset, currRing->cf);
    ...
    else
    {
        ...
        {
            nset = n_Copy(pGetCoeff(set[i].sig), currRing->cf);
            if(!n_GreaterZero(nset, currRing->cf))  n_InpNeg(nset, currRing->cf);
            if(nGreater(nset, np))
                an=i;
            else
...
#endif
{
    ...
    loop
    {
      if (an >= en-1)
      {
        ...
        if(pLmCmp(set[an].sig,p->sig) == 0)
        {
          //gmp_printf("\nSame LM, Not Compare coeff: an = %i, en = %i \n%Zd > %Zd\n",an, en,   pGetCoeff(set[an].sig), pGetCoeff(p->sig));
...
if(rField_is_Ring(currRing))
{
  ...
  if (pLmCmp(sig, strat->syz[strat->syzl-1]) == 1) //1, since this will only be used in global  orderings
        return strat->syzl;
  if (pLmCmp(sig, strat->syz[strat->syzl-1])== 0)
    if(nGreater(pGetCoeff(sig), pGetCoeff(strat->syz[strat->syzl-1]))) 
          return strat->syzl;
  ...
  loop
  {
    if (an >= en-1)
    {
      if(an == en)
        return an;
      if (pLmCmp(strat->syz[an],sig) == 1) 
        return an;
      if(pLmCmp(strat->syz[an],sig) == 0)
      {
        if(nGreater(pGetCoeff(strat->syz[an]), pGetCoeff(sig))) 
          {return an;}
        else
          {return en;}
      }
    ...
    i=(an+en) / 2;
    if (pLmCmp(strat->syz[i],sig) == 1) 
      en=i;
    else
    {
        if(pLmCmp(strat->syz[i],sig) == 0)
        {
            if(nGreater(pGetCoeff(strat->syz[i]), pGetCoeff(sig)))
                en=i;
            else
                an=i;
        }
...
//#if 1
    ...
    printf("\nThese are not the droids (syz) you are looking for.\n");
    return FALSE;
  }
}
...
//#if 1
  ...
  pWrite(sig);
#endif
printf("\nFaugere: sig till k = %i\n",start);pWrite(sig);pWrite(strat->sig[start]);
...
#endif
    if (p_LmShortDivisibleBy(pHead(strat->sig[k]), strat->sevSig[k], pHead(sig), not_sevSig, currRing))
    #ifdef HAVE_RINGS
    if(rField_is_Ring(currRing))
      {if((n_DivBy(pGetCoeff(sig), pGetCoeff(strat->sig[k]), currRing)) && (!pLmEqual(pHead(sig), pHead(strat->sig[k]))))
                {
                #if 1
                printf("\nfaugereRewCriterion: k = %i\n",k);pWrite(sig);pWrite(strat->S[k]);pWrite(strat->sig[k]);
                #endif
        return TRUE;}}
    else
    #endif
...
#endif
    #if ADIDEBUG
    printf("\nRewriten Delete:\n since of Shdl[%i], to delete signature\n",k);pWrite(strat->S[k]);pWrite(strat->sig[k]);pWrite(sig);
...
BOOLEAN arriRewCriterion(poly /*sig*/, unsigned long /*not_sevSig*/, poly /*lm*/, kStrategy strat, int start=0)
{
  ...
  for (int ii=strat->sl; ii>start; ii--)
  {
    ...
    #ifdef HAVE_RINGS
    if(rField_is_Ring(currRing))
      {if(n_DivBy(pGetCoeff(strat->P.sig), pGetCoeff(strat->sig[ii]), currRing))
        {
...
BOOLEAN arriRewCriterionPre(poly sig, unsigned long not_sevSig, poly lm, kStrategy strat, int /*start=0*/)
{
  ...
  for (int ii=strat->sl; ii>-1; ii--)
  {
    ...
    #ifdef HAVE_RINGS
    if(rField_is_Ring(currRing))
      {if(n_DivBy(pGetCoeff(sig), pGetCoeff(strat->sig[ii]), currRing))
          {
        ...
        if(rField_is_Ring(currRing))
          {
            ...
            p_SetCoeff(h.sig, p_GetCoeff(F->m[i],currRing),currRing);
          }
          else
        #endif
        p_ExpVectorAdd (h.sig,F->m[i],currRing);
...
// #if 0
  ...
  {
    if (p.sevSig == 0)
      p.sevSig = pGetShortExpVector(pHead(p.sig));
    else
      assume(p.sevSig == pGetShortExpVector(pHead(p.sig)));
...
void ReduceCoefInitial(kStrategy &strat)
{
  ...
  if(nCoeff_is_Ring_Z(currRing->cf))
  {
    ...
    {
        poly pp = strat->S[ii];
        if(!nEqual(strat->coefred, pGetCoeff(pp)) || (p_Deg(pp, currRing) != 0))
          ReduceCoef(pp, TRUE, strat);
        /*if(!nEqual(strat->coefred, pGetCoeff(pp)) || (p_Deg(pp, currRing) != 0))
        {
...
void ReduceCoef(poly &p, bool FromInitial, kStrategy &strat)
{
    ...
    if(strat->coefred != NULL)
    {
        ...
        if((p_Deg(p, currRing) == 0) && !FromInitial)
        {
            p->coef = nGcd(strat->coefred,pGetCoeff(p));
            strat->coefred = nCopy(pGetCoeff(p));
            ReduceCoefInitial(strat);
            return;
        }
        if(pNext(p) == NULL)
        {
          p->coef = currRing->cf->cfIntMod(pGetCoeff(p), strat->coefred, currRing->cf);
          if(nIsZero(p->coef))
          {  
            p = NULL;
          }
...
void postReduceByMonSig(LObject* h, kStrategy strat)
{
  ...
  for(int i = 0; i<=strat->sl; i++)
  {
    ...
    if(pNext(strat->S[i]) == NULL)
    {
      if(pLmDivisibleBy(strat->S[i], p))
      {
        SigMult = pCopy(pHead(p));
        pExpVectorSub(SigMult, pHead(strat->S[i]));
        pSetCoeff(SigMult,nDiv(p->coef, strat->S[i]->coef));
        printf("\nThis is p, S, SigMult\n");pWrite(p);pWrite(strat->S[i]);pWrite(SigMult);pWrite(h->sig);pWrite(strat->sig[i]);
        if(!nIsZero(SigMult->coef))
        {
            p->coef = currRing->cf->cfIntMod(p->coef, strat->S[i]->coef, currRing->cf);
            h->sig = pMinus_mm_Mult_qq(h->sig, SigMult, strat->sig[i]);
            if(nIsZero(pGetCoeff(h->p)) && pNext(h->p) == NULL)
            {
              printf("\nIch bin hier: \n");
              h->p = NULL;
              h->sig = NULL;
        ...
        printf("\nThis is hSig after\n");if(!nIsZero(h->sig->coef)) pWrite(h->sig);
      }
      pp = pNext(p);
      while(pp != NULL)
      {
        if(pLmDivisibleBy(strat->S[i], pp))
        {
          SigMult = pCopy(pHead(pp));
          pExpVectorSub(SigMult, pHead(strat->S[i]));
          pSetCoeff(SigMult,nDiv(pp->coef, strat->S[i]->coef));
          if(!nIsZero(SigMult->coef))
          {
            pp->coef = currRing->cf->cfIntMod(pp->coef, strat->S[i]->coef, currRing->cf);
            h->sig = pMinus_mm_Mult_qq(h->sig, SigMult, strat->sig[i]);
  ...
  h->sev = p_GetShortExpVector(pHead(h->p),currRing);
  h->SetLmCurrRing();
  if(deleted)
    strat->initEcart(h);
}
